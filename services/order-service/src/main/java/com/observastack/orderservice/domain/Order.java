 ...
 * transition: an order always has at least one line item, and its
 * {@link OrderStatus} only moves along the transitions allowed by
 * {@link #place} and {@link #cancel}. There is no setter — once built,
 * an {@link Order} cannot be pushed into a state its own rules wouldn't
 * allow it to reach on its own.
 */
public final class Order {
     ...
     */
    public static Order create(CustomerId customerId, List<OrderLineItem> lineItems, Clock clock) {
        Objects.requireNonNull(clock, "clock must not be null");
        return new Order(OrderId.newId(), customerId, lineItems, OrderStatus.CREATED, Instant.now(clock), null, null);
    }
    ...
    public static Order reconstruct(
            ...
            Instant placedAt,
            Instant cancelledAt) {
        return new Order(id, customerId, lineItems, status, createdAt, placedAt, cancelledAt);
    }
    ...
    public Money totalPrice() {
        return lineItems.stream().map(OrderLineItem::lineTotal).reduce(Money.zero(), Money::add);
    }
